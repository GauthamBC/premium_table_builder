...
@media(max-width:720px){{body{{padding:10px}}header{{grid-template-columns:1fr;padding:20px 18px 18px}}header img{{max-width:150px;max-height:38px}}.body{{padding:12px}}.search{{max-width:none;flex-basis:100%}}.pager{{margin-left:0;width:100%;justify-content:flex-end}}.footer-main{{flex-direction:column;align-items:flex-start}}.footer-meta div{{grid-template-columns:72px minmax(0,1fr)}}}}
{mobile_css}
...
with left:
    ...
    with st.container(border=True):
        ...
        with b:
            sticky = st.toggle("Sticky header", True); row_count = st.toggle("Row count", True); top3 = st.toggle("Highlight top 3", True); mobile_cards = st.toggle("Mobile cards", True)
